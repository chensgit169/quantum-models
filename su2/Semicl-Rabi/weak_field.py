...
def demo_avoided_crossing():
    g = 1
    plt.figure(figsize=(8, 6))
    d_vals = np.linspace(2.85, 2.96, 201)
    e_vals = np.array([quasi_energy(g, d) for d in d_vals])
    plt.plot(d_vals, e_vals)
    plt.hlines(y=-0.5, xmin=np.min(d_vals), xmax=np.max(d_vals), colors='gray', linestyles='dashed', alpha=0.5)
    plt.xlabel(r'$\Delta/\omega$')
...
def plot_results(g, d_vals, e_vals, approx_1st, approx_2nd, approx_3rd):
    ...
    # plt.plot(d_vals, approx_1st, label='1st Magnus', linestyle='--')
    # plt.plot(d_vals, approx_2nd, label='2nd Magnus', linestyle='-.')
    # plt.plot(d_vals, approx_3rd, label='3rd Magnus', linestyle=':')
...
def direct_magnus():
    ...
    a1_m = np.array([a1_integral(f, 0, 2 * np.pi, g, d) for d in d_vals])
    c2_m = np.array([c2_integral(f, 0, 2 * np.pi, g, d) for d in d_vals])
    a3_m = np.array([a3_integral(f, 0, 2 * np.pi, g, d) for d in d_vals])
    ...
    from matplotlib.patches import Rectangle
    rect = Rectangle((1.6, -0.1), 0.5, 0.2,
                     linewidth=1,
                     edgecolor='red',
                     facecolor='none',
                     alpha=0.8)
    ...
    plt.savefig("figures/quasienergy/weak_field/"+
                f"direct_magnus_g={g}_MA.pdf", dpi=400)
    plt.show()
